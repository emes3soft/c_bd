...
class TABELAS:
    ...
    def verifica_tabelas_existem(self):
        ...
        for t in tabelas:
            ...
            cmd = TESTE_EXISTE_TABELA.replace("__base_dados__", self.con.ligacao.dicionario["database"]).replace("__tabela__", t[0])
        ...
        if self.__tabelas__.num_incorretas > 0:
            cria_mensagem(1011, f"{i.stack()[1][3].upper()} - {self.__class__.__name__}")
    ...
    class TABELA_UNICA():
        ...
        def cria_estrutura_tabela(self):
            cmd = DEVOLVE_CAMPOS_TABELA.replace("__base_dados__", self.con.ligacao.dicionario["database"]).replace("__tabela__", self.nome)

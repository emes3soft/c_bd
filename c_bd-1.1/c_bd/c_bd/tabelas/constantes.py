...
DEVOLVE_CAMPOS_TABELA :str = """
    SELECT A.TABLE_NAME as tabela, A.COLUMN_NAME as nome, A.ORDINAL_POSITION as posicao, A.COLUMN_DEFAULT as valor_por_defeito, 
    (CASE WHEN A.IS_NULLABLE = 'NO' THEN FALSE ELSE TRUE END) as nulo, 
    A.DATA_TYPE as tipo_dados, A.CHARACTER_MAXIMUM_LENGTH as comp_max_texto, 
    A.NUMERIC_PRECISION as precisao_numerica, A.NUMERIC_SCALE as precisao_decimal, A.DATETIME_PRECISION as precisao_data, 
    (CASE WHEN A.COLUMN_KEY = 'PRI' THEN TRUE ELSE FALSE END) as chave_primaria,
    (CASE WHEN A.EXTRA = 'AUTO_INCREMENT' THEN TRUE ELSE FALSE END) as auto_incremento, 
    (CASE WHEN A.COLUMN_KEY = 'MUL' THEN TRUE ELSE FALSE END) as chave_externa,
    B.REFERENCED_TABLE_NAME AS chave_externa_tabela, 
    B.REFERENCED_COLUMN_NAME AS chave_externa_campo
    ...
    WHERE A.TABLE_SCHEMA = '__base_dados__' AND A.TABLE_NAME IN ('__tabela__') 
